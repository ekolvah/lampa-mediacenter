...
```bash
adb shell su 0 cat /data/local/tmp/jacred-boot.log   # что сделал boot-хук
adb shell su 0 pidof JacRed                          # жив ли процесс
...
после загрузки, в лог уходит `GIVING UP` с причиной. Успешный хук выглядит так:

```
07:29:24 boot hook started
07:29:24 chroot на месте, стартуем JacRed
07:29:27 jacred-start.sh rc=0: started pid=3590
```

Две ловушки, обе стоили времени именно после перезагрузки:

- **Диагностику писать в форме `su 0 <команда>`, а не `su -c '…'`.** В первые
  минуты после загрузки вторая форма отдаёт `uid=0` и полный `CapEff`, но
  файловые операции идут от `shell`: каталоги с режимом `700`
  (`/data/debian/opt/jacred`, `/data/debian/root`) отвечают `Permission denied`,
  а `755` читаются — и выглядит это как поломка установки, хотя всё цело.
  `su 0 …` и `su -mm -c '…'` работают сразу.
- **Пустой `logcat` ничего не доказывает.** Кольцевой буфер во время загрузки
  прокручивается за минуты, и записи `su.d`/`jacred-boot` из него уходят.
  Первичный источник — `jacred-boot.log`, он на диске.
...
Скорость замерена на устройстве: около 2,7 суток исходного времени за 15 минут
работы, догон от 1 до 18 августа занял примерно два часа. Идёт неравномерно —
на плотных участках пачка в 2000 раздач продвигает курсор всего на минуты.
Суточный проход добирает сутки за минуту, так что заметно это только на первом
догоне после переливки дампа.

Проход закончился, когда в последней пачке `nextread=False`, а следом идёт
итоговая строка:

```
sync: end / 2026-08-21 04:30:53 (cycle added 5947 torrents in 00:00:43.2)
```
